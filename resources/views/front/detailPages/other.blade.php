...
@section("content")
    ...
    font-weight: 400;
    font-style: normal;">Diğer Afişler Arşivi</h2>
        </div>
        <div class="social-poster-title ">
            <strong><p> {{$other_poster->count()}} Afiş</p></strong>
        </div>
    </div>
            ...
            <article class="poster-item ">
                @foreach($other_poster as $other)
                    <div href="" class="poster-items- w-imgs">
                        <div class="poster-card-top">
                            <picture>
                                <img
                                    src="{{asset('uploads/thumbnail/'.$other->image)}}"
                                    alt="">
                            </picture>
                        </div>
                ...
                <span class="common-links">
                    <h2 class="title"><strong> <!--Tasarımcı adı--->
                            <span><a href="{{route('designer.profile',$other->getUser->id)}}"
                                     class="">{{($other->getUser->name)}}</a></span></strong>
                    </h2>
                    <h2 class="title modal-sub-title" ><strong> <!--Proje adı--->
                            <span><a class="">{{($other->title)}}</a></span></strong>
                    </h2><!---->
                    <h1 class="title">{{$other->date = date('d-m-Y', strtotime($other->date))}}</h1> <!--Tarih--->
                    <div style="white-space:nowrap;
width: 200px;
overflow: hidden;
text-overflow: ellipsis;">
                        <small  class="caption lining-numbers">{{($other->country)}} |</small>
                        <!--Kullanıldığı yer-->
                        <small class="caption lining-numbers">{{($other->printing_technique)}},</small>
                        <!--Baskı tekniği-->
                    </div>
                    <small class="caption lining-numbers">{{($other->dimensions)}}</small><!--ebat-->
                            ...
                            <div class="afis-detail">
                                ...
                                <button type="button" class="btn btn- border" onclick="getPoster({{$other->id}})"  data-toggle="modal"
